...
2. For each product, extract or estimate multi-marketplace pricing (Amazon, Flipkart, Tata CLiQ, Croma, Myntra, Brand Store).
3. Generate BuyHatke-style 30-to-90 day price intelligence:
   ...
   - "priceDropChance": percentage (e.g. 20 for stable/low, 75 for overpriced/sale coming)
   - "priceDropPrediction": 1-sentence prediction (e.g., "Currently at 60-day lowest price. Low chance of further drop." or "Historically drops by ₹500 during monthly sales.")
...
[
  {
    ...
    "stores": [
      {
        "name": "<e.g. Amazon, Flipkart, Tata CLiQ, Titan.co.in, Myntra>",
        "price": <integer price>,
        "inStock": true,
...
=== CRITICAL RULES ===
- Only return products that MATCH the user's requested item: "${mission.searchTerm}". Do NOT return laptops when user asked for watches!
...
/**
 * Sanitize product data ensuring all fields and BuyHatke price intelligence are present
 */
function sanitizeProduct(p, idx, mission) {
  ...
  return {
    ...
    stores: (p.stores && p.stores.length > 0) ? p.stores.map(s => ({
      ...
      { name: 'Amazon India', price, isBest: true, inStock: true, delivery: 'Tomorrow', returnDays: 7, link: '#' }
    ],
    priceHistory: {
      lowest30Days: lowest,
